...
from app.utils.data_loader import (
    ...
    detect_data_type,
    load_sample_data,
    generate_sample_procurement_data,
    generate_sample_logistics_data,
    generate_sample_supplier_data,
)
from app.utils.emissions_calc import (
    calculate_logistics_emissions,
    calculate_procurement_emissions,
...
from app.utils.optimization import (
    ...
    generate_sustainable_procurement_plan,
    generate_supplier_sustainability_plan,
    generate_ai_sustainability_strategy,
)
...
from app.models.risk_assessment import (
    calculate_supplier_risk_scores,
...
from app.dashboard.components import (
    create_sidebar,
    render_emissions_summary,
    render_optimization_summary,
    render_supplier_assessment,
    render_compliance_summary,
    render_kpi_metrics,
    render_ai_chat_interface,
    render_data_upload_interface,
...
def show_dashboard_page():
    ...
    st.markdown(
        '<h1 class="main-header">Supply Chain Sustainability Dashboard</h1>',
        unsafe_allow_html=True,
    )
    st.image("app/assets/images/logo.png", width=250)

    # Check if data is available
    data_available = (
        st.session_state.procurement_data is not None
        or st.session_state.logistics_data is not None
        or st.session_state.supplier_df is not None
    )
    ...
    if not data_available:
        ...
        with col1:
            st.image(
                "https://cdn-icons-png.flaticon.com/512/6519/6519723.png", width=250
            )

        with col2:
            st.markdown(
                """
            <h2 style="color: #1E6797;">Welcome to EcoChainAI!</h2>
            ...
            st.button(
                "🚀 Load Sample Data", key="load_sample", on_click=load_sample_dataset
            )
        ...
        st.markdown(
            '<h3 class="subheader">Getting Started</h3>', unsafe_allow_html=True
        )
    ...
    with col2:
        if st.session_state.total_emissions is not None:
            total_tonnes = st.session_state.total_emissions.get(
    ...
    with col3:
        if st.session_state.optimization_targets is not None:
            reduction = st.session_state.optimization_targets.get(
                "potential_reduction_percent", 0
            )
            st.metric(
                "Reduction Potential", f"{reduction:.1f}%", delta=f"-{reduction:.1f}%"
            )
        else:
    ...
    with col1:
        ...
        if st.session_state.total_emissions is not None:
            st.markdown(
                '<h3 class="subheader">Emissions Overview</h3>', unsafe_allow_html=True
            )
            st.markdown('<div class="card">', unsafe_allow_html=True)
    ...
    with col2:
        ...
        if st.session_state.optimization_targets is not None:
            st.markdown(
                '<h3 class="subheader">Optimization Opportunities</h3>',
                unsafe_allow_html=True,
            )
            st.markdown('<div class="card">', unsafe_allow_html=True)
    ...
    st.markdown(
        '<h3 class="subheader">AI-Generated Sustainability Strategy</h3>',
        unsafe_allow_html=True,
    )
    ...
    if st.session_state.ai_strategy is not None:
        with st.expander("View Strategy", expanded=True):
            st.markdown(
                '<div class="card" style="background-color: #f8fdf9;">',
                unsafe_allow_html=True,
            )
            ...
            if st.button("🤖 Generate Strategy", key="gen_strategy"):
                with st.spinner("Generating sustainability strategy..."):
                    strategy = generate_ai_sustainability_strategy(
                        ...
                        optimization_targets=st.session_state.get(
...
def show_data_upload_page():
    ...
    if (
        ...
        with col1:
            if st.session_state.procurement_data is not None:
                st.write(
                    f"**Procurement Data:** {len(st.session_state.procurement_data)} records"
                )
            else:
        ...
        with col2:
            if st.session_state.logistics_data is not None:
                st.write(
                    f"**Logistics Data:** {len(st.session_state.logistics_data)} records"
                )
            else:
        ...
        with col3:
            if st.session_state.supplier_df is not None:
                st.write(
                    f"**Supplier Data:** {len(st.session_state.supplier_df)} records"
                )
            else:
...
def show_emissions_analysis_page():
    ...
    emissions_tab1, emissions_tab2, emissions_tab3 = st.tabs(
        ["Logistics", "Procurement", "Data Tables"]
    )
    ...
    with emissions_tab1:
        if st.session_state.logistics_emissions is not None:
            ...
            if emissions_by_mode:
                ...
                mode_df["Emissions (tonnes CO₂e)"] = (
                    mode_df["Emissions (kg CO₂e)"] / 1000
                )

                # Create bar chart
                st.bar_chart(
                    mode_df.set_index("Transport Mode")["Emissions (tonnes CO₂e)"]
                )
        else:
        ...
        if st.session_state.procurement_emissions is not None:
            ...
            if emissions_by_category:
                ...
                category_df = category_df.sort_values(
                    "Emissions (kg CO₂e)", ascending=False
                )

                # Convert to tonnes for better readability
                category_df["Emissions (tonnes CO₂e)"] = (
                    category_df["Emissions (kg CO₂e)"] / 1000
                )

                # Create bar chart
                st.bar_chart(
                    category_df.set_index("Product Category")["Emissions (tonnes CO₂e)"]
                )
        else:
    ...
    with emissions_tab3:
        ...
        with data_tab1:
            if (
                st.session_state.logistics_data is not None
                and "emissions_kg_co2e" in st.session_state.logistics_data.columns
            ):
        ...
        with data_tab2:
            if (
                st.session_state.procurement_data is not None
                and "emissions_kg_co2e" in st.session_state.procurement_data.columns
            ):
...
def show_optimization_page():
    ...
    if st.session_state.total_emissions is None:
        st.info(
            "Please analyze your emissions first before exploring optimization opportunities."
        )
    ...
    opt_tab1, opt_tab2, opt_tab3 = st.tabs(
        ["Logistics Optimization", "Procurement Optimization", "AI Strategy"]
    )
    ...
    with opt_tab1:
        if st.session_state.logistics_data is not None:
            ...
            if (
                "logistics_optimization" not in st.session_state
                or st.session_state.logistics_optimization is None
            ):
                with st.spinner("Generating logistics optimization..."):
                    optimized_df, summary = generate_optimal_routes(
            ...
            if reduction_by_method:
                ...
                method_df = pd.DataFrame(
                    {
                        ...
                        "Emissions Reduction (kg CO₂e)": list(
                            reduction_by_method.values()
                        ),
                    }
                )
                ...
                method_df = method_df.sort_values(
                    "Emissions Reduction (kg CO₂e)", ascending=False
                )

                # Convert to tonnes for better readability
                method_df["Emissions Reduction (tonnes CO₂e)"] = (
                    method_df["Emissions Reduction (kg CO₂e)"] / 1000
                )

                # Create bar chart
                st.bar_chart(
                    method_df.set_index("Optimization Method")[
                        "Emissions Reduction (tonnes CO₂e)"
                    ]
                )
            ...
            if "optimized_transport_mode" in opt_df.columns:
                ...
                if not changed_shipments.empty:
                    ...
                    display_cols = [
                        col for col in display_cols if col in changed_shipments.columns
                    ]
                    ...
                    st.dataframe(
                        changed_shipments[display_cols].rename(columns=rename_map)
                    )
        else:
    ...
    with opt_tab2:
        if st.session_state.procurement_data is not None:
            ...
            if (
                "procurement_optimization" not in st.session_state
                or st.session_state.procurement_optimization is None
            ):
                with st.spinner("Generating procurement optimization..."):
                    optimized_df, summary = generate_sustainable_procurement_plan(
            ...
            metrics = {
                ...
                "Reduction": f"{opt_summary.get('percent_reduction', 0):.1f}%",
                "Cost Impact": (
                    f"{opt_summary.get('cost_impact_percent', 0):.1f}%"
                    if opt_summary.get("cost_impact_percent") is not None
                    else "N/A"
                ),
            }
            ...
            if reduction_by_category:
                ...
                # Convert to tonnes for better readability
                category_df["Emissions Reduction (tonnes CO₂e)"] = (
                    category_df["Emissions Reduction (kg CO₂e)"] / 1000
                )

                # Create bar chart
                st.bar_chart(
                    category_df.set_index("Product Category")[
                        "Emissions Reduction (tonnes CO₂e)"
                    ]
                )
            ...
            if "sustainable_alternative" in opt_df.columns:
                ...
                alternatives = (
                    opt_df.groupby(["product_category", "sustainable_alternative"])
                    .agg(
                        {
                ...
                alternatives["emissions_reduction_tonnes"] = (
                    alternatives["emissions_reduction_kg_co2e"] / 1000
                )

                # Format for display
                alternatives_display = alternatives[
                    alternatives["sustainable_alternative"]
                    != "No alternative identified"
                ]
                alternatives_display = alternatives_display.rename(
                    columns={
...
def show_supplier_assessment_page():
    ...
    render_supplier_assessment(
        st.session_state.risk_scores, st.session_state.risk_assessment
    )
    ...
    if (
        "mitigation_strategies" in st.session_state
        and st.session_state.mitigation_strategies
    ):
        for i, strategy in enumerate(st.session_state.mitigation_strategies):
            with st.expander(
                f"{strategy.get('risk_area', 'Strategy')} ({strategy.get('risk_level', 'Medium')} Risk)",
                expanded=i == 0,
            ):
                st.write(f"**Strategy:** {strategy.get('strategy', '')}")
                st.write(
                    f"**Implementation Timeline:** {strategy.get('implementation_timeline', '')}"
                )
                st.write(f"**Expected Impact:** {strategy.get('expected_impact', '')}")
                st.write(
                    f"**Responsible Party:** {strategy.get('responsible_party', '')}"
                )
...
def show_compliance_page():
    ...
    if (
        "compliance_report" not in st.session_state
        or st.session_state.compliance_report is None
    ):
    ...
    with col2:
        company_region = st.selectbox(
            ...
            index=[
                ...
                "Brazil",
                "Other",
            ].index(st.session_state.company_region),
        )
    ...
    with col3:
        industry_sector = st.selectbox(
            ...
            index=[
                ...
                "Healthcare",
                "Other",
            ].index(st.session_state.industry_sector),
        )
    ...
    with zip_col1:
        facility_zip_1 = st.text_input(
            "Facility 1 ZIP Code", value=facility_zip_codes[0]
        )

    with zip_col2:
        facility_zip_2 = st.text_input(
            "Facility 2 ZIP Code",
            value=facility_zip_codes[1] if len(facility_zip_codes) > 1 else "",
        )

    updated_zips = [
        zip_code for zip_code in [facility_zip_1, facility_zip_2] if zip_code
    ]
    ...
    if st.button("Regenerate Compliance Assessment"):
        with st.spinner("Regenerating compliance assessment..."):
            report = generate_compliance_report(
                ...
                total_emissions=st.session_state.total_emissions,
                procurement_df=st.session_state.procurement_data,
                facility_zip_codes=st.session_state.get(
                    "facility_zip_codes", ["90210", "10001"]
                ),
            ...
            with st.spinner("Generating PDF report..."):
                pdf_content = generate_pdf_report(
                    ...
                    supplier_df=(
                        ...
                        else st.session_state.supplier_df
                    ),
    ...
    if "html_report" in st.session_state and st.session_state.html_report:
        with st.expander("HTML Report Preview", expanded=True):
            st.components.v1.html(
                st.session_state.html_report, height=600, scrolling=True
            )
...
def show_ai_assistant_page():
    ...
    # Initialize NLP agent if not already done
    ...
    with st.expander("Usage Tips"):
        ...
        - How can I reduce my logistics emissions?
        - Which suppliers have the highest sustainability risk?
        - What regulations am I subject to?
        - How much could I save by optimizing my supply chain?
        - What's the best way to start reducing my emissions?
        - Generate a 12-month sustainability improvement plan.
        - What's the average emissions per shipment?
        """
        )
...
def run_emissions_calculations():
    ...
    if st.session_state.procurement_data is not None:
        procurement_result = calculate_procurement_emissions(
            st.session_state.procurement_data
        )
    ...
    if st.session_state.logistics_data is not None:
        logistics_result = calculate_logistics_emissions(
            st.session_state.logistics_data
        )
    ...
    total_emissions = calculate_total_emissions(
        logistics_summary=(
            st.session_state.logistics_emissions[1]
            if st.session_state.logistics_emissions
            else None
        ),
        procurement_summary=(
            st.session_state.procurement_emissions[1]
            if st.session_state.procurement_emissions
            else None
        ),
        supplier_summary=(
            st.session_state.supplier_emissions[1]
            if st.session_state.supplier_emissions
            else None
        ),
    )
...
def generate_optimization_recommendations():
    ...
    if (
        st.session_state.optimization_targets is None
        and st.session_state.total_emissions is not None
    ):
        optimization_targets = generate_optimization_targets(
    ...
    if st.session_state.supplier_df is not None:
        ...
        if (
            "risk_scores" not in st.session_state
            or st.session_state.risk_scores is None
        ):
            risk_df = calculate_supplier_risk_scores(st.session_state.supplier_df)
            st.session_state.risk_scores = risk_df
        ...
        # Generate supplier optimization
        optimized_df, summary = generate_supplier_sustainability_plan(
            st.session_state.risk_scores
        )
